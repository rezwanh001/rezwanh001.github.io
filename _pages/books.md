...
# -----------------------------------------------------------------
# Reading List Data
# Replace audio_file with the YouTube video ID.
  ...
  <div class="col-lg-3 col-md-6 mb-4 d-flex align-items-stretch">
    <div class="card w-100">
      ...
      <div class="card-body d-flex flex-column">
        ...
        <h6 class="card-subtitle mb-2 text-muted">{{ book.author }} ({{ book.published_date }})</h6>
        <div class="tags-container mb-3">
        {% for tag in book.tags %}
          <span class="badge badge-pill badge-custom">{{ tag }}</span>
        {% endfor %}
        </div>
      ...
      <div class="card-footer bg-transparent border-top-0 text-center">
        <button type="button" class="btn btn-outline-primary btn-sm" data-toggle="modal" data-target="#videoModal{{ forloop.index }}">
...
<!-- =================== MODALS FOR VIDEO PLAYERS ===================== -->
{% for book in page.reading_list %}
  {% if book.youtube_id and book.youtube_id != "" %}
  <div class="modal fade" id="videoModal{{ forloop.index }}" tabindex="-1" role="dialog" aria-labelledby="videoModalLabel{{ forloop.index }}" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="videoModalLabel{{ forloop.index }}">{{ book.title }}</h5>
  ...
  $(document).ready(function() {
    $('.modal').on('hidden.bs.modal', function (e) {
      ...
      if (iframe.length > 0) {
        ...
        // Setting the src to empty stops the video
        iframe.attr('src', '');
        // Restore the original src so the video can be played again
